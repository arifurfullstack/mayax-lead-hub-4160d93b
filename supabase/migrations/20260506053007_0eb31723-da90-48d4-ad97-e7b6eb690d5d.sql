...
BEGIN
  ...
  SELECT COALESCE((SELECT value FROM platform_settings WHERE key='lead_price_credit_score'),'5')::numeric INTO v_credit;

  FOR r IN SELECT id, income, vehicle_preference, trade_in, has_bankruptcy, appointment_time,
                  credit_range_min, credit_range_max, price
  ...
  LOOP
    ...
    IF r.credit_range_min IS NOT NULL OR r.credit_range_max IS NOT NULL THEN
      v_price := v_price + v_credit;
    END IF;
