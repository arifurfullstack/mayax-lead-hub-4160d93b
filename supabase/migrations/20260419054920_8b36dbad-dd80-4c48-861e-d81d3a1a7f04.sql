...
-- Helper to normalize phone (strip non-digits, take last 10)
...
BEGIN
  ...
  IF length(digits) > 10 THEN
    digits := substring(digits FROM length(digits) - 9);
  END IF;
  RETURN digits;
END;
$$;

-- BEFORE INSERT trigger: dedupe leads on email or normalized phone
...
BEGIN
  ...
  IF new_email_norm <> '' THEN
    SELECT id, notes, sold_status
      INTO match_id, match_notes, match_sold_status
      FROM public.leads
  ...
  IF match_sold_status = 'available' THEN
    ...
    WHERE id = match_id;
  ELSE
    -- Already sold: only update notes (and merge documents if provided)
    UPDATE public.leads SET
      notes = appended_notes
    WHERE id = match_id;
  END IF;
...
DROP TRIGGER IF EXISTS dedupe_lead_before_insert_trigger ON public.leads;
CREATE TRIGGER dedupe_lead_before_insert_trigger
BEFORE INSERT ON public.leads
FOR EACH ROW
EXECUTE FUNCTION public.dedupe_lead_before_insert();
