...
interface WalletTopupFailedProps {
...
}: WalletTopupFailedProps) => {
  return (
    <Html lang="en" dir="ltr">
      <Head />
      <Preview>Your wallet top-up of ${Number(amount).toFixed(2)} did not complete</Preview>
      <Body style={main}>
        <Container style={container}>
          ...
          <Section style={content}>
            <Heading style={h1}>Wallet Top-Up Failed ⚠️</Heading>
            <Text style={text}>
              Hi <strong>{dealership_name}</strong>, we were unable to complete your recent
              wallet top-up. Your wallet balance has not been changed.
            </Text>

            <Section style={amountBox}>
              <Text style={amountLabel}>Attempted Amount</Text>
              <Text style={amountValue}>${Number(amount).toFixed(2)}</Text>
            </Section>

            <Section style={summary}>
              <Row style={tr}>
                <Column style={td1}>Reason</Column>
                <Column style={td2}>{reason}</Column>
              </Row>
              <Row style={tr}>
            ...
            <Hr style={hr} />
            <Text style={footer}>
              You can try the top-up again from your Wallet page. If the issue persists,
...
export const template = {
  component: WalletTopupFailedEmail,
  subject: (d: Record<string, any>) =>
    `Wallet top-up failed — $${Number(d?.amount ?? 0).toFixed(2)} not credited`,
  displayName: 'Wallet top-up failed',
  previewData: {
    dealership_name: 'Acme Motors',
    amount: 250,
    gateway: 'stripe',
    reference: 'pr_123',
    reason: 'Stripe session expired (payment_status=unpaid)',
    date: new Date().toLocaleString(),
  },
